...
export class MockApiService {
  ...
  refreshData(): void {
    this.http
      .get<MajorCategory[]>(`${this.baseUrl}/majorCategories`)
      .pipe(
        map(items => items.map(this.normalizeMajor))
      )
      .subscribe(items => this.major$.next(items));

    this.http
      .get<MinorCategory[]>(`${this.baseUrl}/minorCategories`)
      .pipe(
        map(items => items.map(this.normalizeMinor))
      )
      .subscribe(items => this.minor$.next(items));

    this.http
      .get<Expense[]>(`${this.baseUrl}/expenses`)
      .pipe(
        map(items => items.map(this.normalizeExpense))
      )
      .subscribe(items => this.expenses$.next(items));
  }
